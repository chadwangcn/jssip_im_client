...
JsSIP.Transport.prototype = {
  ...
  */
  connect: function() {
    ...
    this.ws.onuserEvent = function(e) {
      transport.onUserEnent(e);
    };
  ...
  */
  onMessage: function(e) {
    ...
    if(message && JsSIP.sanityCheck(message, this.ua, this)) {
	  ...
	  var CT = message.headers["Content-Type"];
      if ( (message.method == JsSIP.c.INFO)  && typeof(CT) != 'undefined' ) {
         $("#sponsor").append(message.body + " " + message.headers["Content-Type"][0]["raw"] + "<br />");
         this.ua.emit('userEvent', this.ua, message);
      }
...
JsSIP.Session.prototype.applyVideoChannel = function(target,userid,options,onSuccess) {
	...
	var event, eventHandlers, request, selfView, remoteView, mediaType, extraHeaders, requestParams;
	options = options || {};
	selfView = options? options.selfView : null;
	remoteView = options? options.remoteView : null;
	mediaType = options.mediaType || {audio: true, video: true};	
...
JsSIP.Session.prototype.StopVideoChannel = function(target,userid) {
	...
	this.mediaSessionExt[userid].peerConnection.close();
  ...
  */
  start: function(onSuccess) {
    ...
    this.peerConnection.onaddstream = function(mediaStreamEvent) {
	  ...
	  console.warn('getRemoteStreams');
      }
	  
	  var sdp =this.localDescription.sdp;

      if (session.remoteView && test > 0) {
		console.warn('remoteView test=%d localsdp=%s  \n', test,sdp);
